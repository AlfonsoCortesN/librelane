...
# macro grid, hierarchical method
define_pdn_grid -macro -name {macro_grid} -voltage_domains {CORE} -instances {DUT}
add_pdn_stripe -grid {macro_grid} -layer {TopMetal1} -width {3} -pitch {20} -offset {1.0}
add_pdn_connect -grid {macro_grid} -layers {TopMetal1 TopMetal2} 
add_pdn_connect -grid {macro_grid} -layers {Metal5 TopMetal1}

# macro grid, ring method
# define_pdn_grid -macro -name {macro_grid} -voltage_domains {CORE} -instances {DUT}
# add_pdn_ring -grid {macro_grid} -layers {Metal5 TopMetal1} -widths {5.0} -spacings {5.0} -core_offsets {4.5}
# add_pdn_connect -grid {macro_grid} -layers {TopMetal1 TopMetal2} 
# add_pdn_connect -grid {macro_grid} -layers {Metal5 TopMetal1}