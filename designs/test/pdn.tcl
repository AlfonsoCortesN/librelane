...
add_pdn_stripe -grid {grid} -layer {TopMetal1} -width {5.000} -pitch {80} -offset {15} -extend_to_core_ring
add_pdn_stripe -grid {grid} -layer {TopMetal2} -width {5.000} -pitch {80} -offset {15} -extend_to_core_ring
...
define_pdn_grid -macro -name {sram_grid} -voltage_domains {CORE} -cells {RM_IHPSG13_1P_64x64_c2_bm_bist}
add_pdn_stripe -grid {sram_grid} -layer {TopMetal1} -width {3} -pitch {20} -offset {1.5} -starts_with GROUND
